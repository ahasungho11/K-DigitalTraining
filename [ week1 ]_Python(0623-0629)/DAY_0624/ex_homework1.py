...
def showAddress():
    ...
    for addr in ADDR_LIST:
        print(addr)
# ADDR_LIST를 대체해서 (직접 엑세스 가능하니까, 모듈을 써서)
# os.listdir(DIR_PATH)써주면서

# addr[:-4]해서 이런식으로 제외하고 뽑아오면 됨
...
def searchAddress(name_phone):
    ...
    for add in ADDR_LST:
        if name_phone in add:
            print(f'파일명 : {add}')
            with open(DIR_PATH+add+'.txt', mode = 'r', encoding='etf-8') as f:
                print(f'정  보 : {f.read()}')
...
# 프로그램 초기화 함수-------------- 모듈 쓰는 것에 따라 여부 결정해도 됨
...
def initApp():
    ...
    #     print(f.write())
    # => 모듈을 써서 해결
# 개인주소록 저장할 공감 (txt로 만든 것)

# if not os.path.extsep(DIR_PATH) : os.mkdir(DIR_PATH)
# a나 w로 파일은 생성할 수 있으니, ,
# 첨에만 존재 여부를 체크하도록 하면 되겠지?
...
while True:
    ...
    if select == '4': break
        ...
        search = input("이름, 전화번호, 지역, 이메일 : ex) 홍길동, 123-4567-8901, 대구, 이메일 ").split(',')
