...
class poseDetector():
    ...
    def findPosition(self, img, draw=True):
        lmlist = [] # 랜드마크용 리스트 만들기
        if self.results.pose_landmarks:
            for id, lm in enumerate(self.results.pose_landmarks.landmark):
                h, w, c = img.shape
                # print(id, lm)
                # lm을 쓰면 무슨일이 일어나는지 볼 수 있고, id번호도 출력할 수 있음
                ...
                # x좌표*너비, y좌표*높이
                # lmlist.append([id, cx, cy])#x, y, z, id 추가
                lmlist.append([id, cx, cy, int(cz)])  # x, y, z, id 추가
        ...
        return lmlist  # 33개의 점의 x,y,z를 뽑아낼 수 있음 => 원하는 숫자를 뽑아낼 수 있다
...
def main():
    cap = cv2.VideoCapture('test.mp4')  # test, 1, 3(판넬), 6(하천), 7 (전체 : ~7)
    pTime = 0
    ...
    while True:
        ...
        if len(lmlist) != 0:
            print(lmlist)
            # print(lmlist[2], lmlist[25])
            # cv2.circle(img, lmlist[14], lmlist[14], 15, (0, 0, 255), cv2.FILLED)
