...
RANKING = 10
def get_top20_kospi_index(url, company_url_list, company_name_list):
    '''
...
def get_company_info(company_url):
    ...
    for item in dd_items:
        item_list = item.text.split()    # 공백 기준으로 분리
        key = item_list[0]               # 항목이름 : key
        value = item_list[1]             # 항목에 대한 값
        # stock_dict 딕셔너리의 key에 해당하는 내용만 딕셔너리에 저장
        if key in stock_dict_keys:
            stock_dict[key] = value      # 딕셔너리에 데이터 추가
...
def main():
    ...
    top_url_list = [] # top20 회사의 Naver url 정보
    top_company_list = [] # top20 회사의 이름

    get_top20_kospi_index(naver_stock_url, top_url_list, top_company_list)
    #print(top_url_list)
